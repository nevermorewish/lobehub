...
- 在 `/prices` 新增或编辑模型时，选择服务商后点击「拉取服务商模型」。后台使用已保存的密钥请求上游，支持 OpenAI 兼容、Anthropic、Google 和 Ollama 列表；Azure/Bedrock 暂需手填模型 ID。拉取只提供候选，选择后核对模型类型、能力和价格，保存启用后才对前端发布。
