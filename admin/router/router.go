...
func New(s *service.Service, web fs.FS) *gin.Engine {
	...
	admin.GET("/settings/:section", h.Settings)
	admin.PUT("/settings/:section", h.SaveSettings)
	admin.POST("/deployments/probe", h.ProbeDeployment)
	admin.GET("/deployments", h.Deployments)
	admin.POST("/deployments", h.StartDeployment)
	admin.GET("/deployments/:id", h.DeploymentStatus)
	...
	internal.GET("/settings", h.RuntimeSettings)
